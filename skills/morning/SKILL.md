...
**Agents first.** Before any connector call, get the tracked-work view from the agents in `~/.claude/agents/`. Always spawn `atlas` — chief of staff, owns continuity across the studio, the clients, and 36 Oak St — and ask it for four things: what is open, what is overdue, what moved since the last brief, and any earlier conclusion that has since been overturned. Add `rafael` (client delivery and phase gates), `vera` (paid-media checks due), `marcus` (property and filing deadlines), and `june` (the vacant unit) when the day plausibly touches them. Send them in one message so they run concurrently, and let a slow agent be dropped rather than delay the page.

What comes back are candidates for the two lists below, ranked against connector items and held to the same Needs attention test — an agent's opinion is not an item; the commitment underneath it is. Atlas's memory directory is the record for dated commitments, so a deadline it surfaces outranks an inbox item landing the same day. An agent that returns nothing is skipped silently, and if no agents are defined the page falls through to connectors alone.

The point of this section is momentum: the brief should leave a clear thread to pick up, not just a list of what exists.
